...
export default function SpaceModerationPage({ params }: { params: Promise<{ spaceId: string }> }) {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Fetch space info
      const spaceResponse = await fetch(`/api/admin/spaces/${spaceId}`, {
        headers: {
      ...
      const reportsResponse = await fetch(`/api/admin/spaces/${spaceId}/moderation?status=${statusFilter}`, {
        headers: {
          'Authorization': `Bearer ${admin.id}`,
        },
      });
