...
export default function SchoolAdminInvitesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      // Fetch school admins
      const adminsResponse = await fetch('/api/admin/school-admins', {
        headers: {
      ...
      const campusesResponse = await fetch('/api/admin/schools', {
        headers: {
          'Authorization': `Bearer ${admin.id}`,
        },
      });
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch('/api/admin/school-admins/invite', {
        method: 'POST',
        headers: {
  ...
  const handleRevoke = async (adminId: string) => {
    ...
    setActionLoading(true);
    try {
      const response = await fetch(`/api/admin/school-admins/${adminId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${admin.id}`,
        },
