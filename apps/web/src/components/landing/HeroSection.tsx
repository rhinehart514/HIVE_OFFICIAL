          ...
          <div className="w-full max-w-[380px]">
            ...
            <div className="rounded-2xl bg-[#0A0A0A] border border-white/[0.08] p-6">
              ...
              <div className="space-y-2.5">
                <PollOption label="Event coordination" percent={42} active />
                <PollOption label="Member signups" percent={28} />
                <PollOption label="Polls & voting" percent={18} />
                <PollOption label="Countdowns & reminders" percent={12} />
              </div>
...
function PollOption({
  ...
  return (
    <div className="relative overflow-hidden rounded-lg bg-white/[0.03] px-4 py-3">
      {/* Fill bar */}
      <div
      ...
      <div className="relative flex items-center justify-between">
        <span className={`text-[13px] ${active ? 'text-white font-medium' : 'text-white/60'}`}>
          {label}
        </span>
        <span className={`text-[13px] font-mono ${active ? 'text-[#FFD700]' : 'text-white/40'}`}>
          {percent}%
        </span>
      </div>
    </div>
