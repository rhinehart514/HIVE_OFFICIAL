      ...
      <div className="min-h-screen flex items-center justify-center bg-neutral-950">
        <div className="flex flex-col items-center gap-4">
          <div className="animate-pulse">
            <HiveLogo className="w-10 h-10 text-[var(--hive-gold)]" />
          </div>
  ...
  if (!currentUser && !sessionLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center px-4">
        ...
        >
          <div className="w-16 h-16 mx-auto mb-6 rounded-2xl bg-[var(--hive-gold)]/10 flex items-center justify-center">
          ...
          <Button onClick={() => router.push('/enter')} className="rounded-full px-6 bg-life-gold text-ground hover:bg-life-gold/90">
            Sign In
          </Button>
        </motion.div>
      </div>
    ...
    <div className="min-h-screen pb-20">
          ...
          <div className="flex items-center justify-between h-16">
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center gap-3">
                <Avatar className="h-9 w-9 ring-2 ring-life-gold/30">
                  <AvatarImage src={profileData.profile.avatarUrl ?? undefined} alt={profileData.profile.fullName} />
                  <AvatarFallback className="text-sm bg-neutral-800 text-white">{initials}</AvatarFallback>
                </Avatar>
            ...
            <div className="sm:hidden flex items-center gap-2 text-life-gold">
            ...
            <div className="flex items-center gap-2">
              <Button onClick={handleCancel} variant="ghost" className="hidden sm:flex text-neutral-400 hover:text-white">
                Cancel
              </Button>
              <Button
                onClick={handleDone}
                disabled={isSaving}
                className="rounded-full px-5 bg-life-gold text-ground hover:bg-life-gold/90 font-medium disabled:opacity-50"
              >
                {isSaving ? (
      ...
      >
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2 flex items-center gap-6">
            <div className="relative">
              <Avatar className="h-24 w-24 ring-2 ring-neutral-800">
                ...
                <AvatarFallback className="text-2xl bg-neutral-900 text-white">{initials}</AvatarFallback>
              </Avatar>
              <label className="absolute -bottom-1 -right-1 p-2 rounded-full bg-life-gold text-ground cursor-pointer hover:bg-life-gold/90 transition-colors">
                {isUploadingAvatar ? (
          ...
          {/* Handle (readonly) */}
          <div>
            <label className="block text-sm font-medium text-neutral-400 mb-2">Handle</label>
            <div className="flex items-center gap-2">
              ...
              <Button variant="ghost" size="sm" onClick={() => router.push('/settings')} className="text-neutral-400 hover:text-white">
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              ...
              rows={3}
              maxLength={200}
              className="w-full px-3 py-2 rounded-md bg-surface border border-zinc-700 text-white placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-white/20 resize-none"
            />
          ...
          {/* Major */}
          <div>
            <label className="block text-sm font-medium text-neutral-400 mb-2">Major / Program</label>
            <Input
              value={major}
          ...
          {/* Interests */}
          <div className="sm:col-span-2">
            <label className="block text-sm font-medium text-neutral-400 mb-2">Interests ({interests.length}/10)</label>
            <div className="flex flex-wrap gap-2 mb-3">
              {interests.map((interest) => (
                <span key={interest} className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-sm bg-neutral-800 text-neutral-300 border border-neutral-700">
                  {interest}
                  <button onClick={() => handleRemoveInterest(interest)} className="p-0.5 rounded-full hover:bg-neutral-700 transition-colors">
      ...
      <motion.div
        ...
        className="mx-auto max-w-6xl px-4 sm:px-6 py-8 border-b border-zinc-800"
      >
        ...
        <div className="rounded-xl border border-zinc-800 bg-surface p-6">
          <div className="flex items-start gap-4">
            <div className="flex-shrink-0 w-12 h-12 rounded-xl bg-zinc-800 flex items-center justify-center">
              <EyeSlashIcon className="w-6 h-6 text-zinc-400" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-1">
                <h3 className="text-base font-medium text-white">Ghost Mode</h3>
                {ghostModeState?.enabled && (
                  <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-life-gold/10 text-life-gold">Active</span>
                )}
              </div>
              ...
              <Button onClick={() => setIsGhostModeModalOpen(true)} variant="secondary" className="border-zinc-700 text-white hover:bg-zinc-800">
      ...
      <motion.div
        ...
        className="bg-gradient-to-r from-life-gold/10 via-orange-500/10 to-life-gold/10 border-b border-life-gold/20"
      >
        <div className="mx-auto max-w-6xl px-4 sm:px-6 py-4">
          <div className="flex items-center gap-3">
            <div className="flex-shrink-0 w-10 h-10 rounded-xl bg-life-gold/20 flex items-center justify-center">
              <Bars3Icon className="w-5 h-5 text-life-gold" />
            </div>
            <div>
              <h2 className="text-sm font-medium text-white">Customize your layout</h2>
              <p className="text-xs text-neutral-400">Drag and resize tiles to personalize your profile grid.</p>
            </div>
          </div>
        </div>
      </motion.div>
      ...
      {hasPendingChanges && (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
          <div className="flex items-center gap-3 px-4 py-3 rounded-full bg-neutral-900 border border-neutral-700 shadow-xl">
            <div className="w-2 h-2 rounded-full bg-life-gold animate-pulse" />
            <span className="text-sm text-neutral-300">Unsaved changes</span>
            <Button
              onClick={handleSaveLayout}
              disabled={isSaving}
              size="sm"
              className="rounded-full px-4 bg-life-gold text-ground hover:bg-life-gold/90 text-xs font-medium"
            >
