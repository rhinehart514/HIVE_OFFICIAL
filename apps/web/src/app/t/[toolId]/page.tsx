 ...
 *
 * Public shareable tool page. Works without authentication for public tools.
 * Minimal chrome, mobile-first, with tool attribution.
 *
 * Features:
 * - No space context required
 * - Works for both logged-in and anonymous users
 * - Clean, focused tool experience
 * - Builder attribution (name, avatar, campus if present)
 * - Optional login prompt for private tools
  ...
  try {
    ...
    return {
      ...
      openGraph: {
        ...
        url: `https://hive.com/t/${toolId}`,
        siteName: 'HIVE',
        images: [
          {
            url: `https://hive.com/api/og/tool/${toolId}`,
            width: 1200,
            height: 630,
            alt: tool.name,
          },
      ...
      twitter: {
        ...
        title: tool.name,
        description: tool.description || `Built by ${tool.ownerName || 'a HIVE user'}`,
        images: [`https://hive.com/api/og/tool/${toolId}`],
      },
    };
  } catch {
    return {
...
export default function StandaloneToolPage({ params }: Props) {
