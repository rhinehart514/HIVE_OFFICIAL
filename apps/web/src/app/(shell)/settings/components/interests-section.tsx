      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {localInterests.length === 0 ? (
          ...
          localInterests.map((interest) => (
            <span
              key={interest}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-white/[0.05] text-sm text-white"
            >
              {interest}
              <button
                onClick={() => handleRemoveInterest(interest)}
                className="p-0.5 rounded hover:bg-white/[0.10] transition-colors"
        ...
        <div className="relative">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2 bg-white/[0.05] border border-white/[0.05] rounded-lg text-white placeholder:text-white/50 focus:border-white/50 focus:outline-none"
            />
            <Button
              variant="secondary"
          ...
          {showSuggestions && filteredSuggestions.length > 0 && (
            <div
              ref={suggestionsRef}
              className="absolute top-full left-0 right-0 mt-1 py-1 bg-[var(--bg-surface-hover)] border border-white/[0.05] rounded-lg z-50 max-h-48 overflow-y-auto"
            >
              {filteredSuggestions.map((suggestion) => (
                <button
                  key={suggestion}
                  onClick={() => handleAddInterest(suggestion)}
                  className="w-full px-3 py-2 text-left text-sm text-white/50 hover:text-white hover:bg-white/[0.10] transition-colors"
                >
                  {suggestion}
                </button>
              ))}
