...
export function useFeed(options: FeedOptions = {}): UseFeedReturn {
  ...
  const loadPosts = useCallback(async (reset = false) => {
    ...
    try {
      ...
      logger.error('Feed load error', { component: 'use-feed' }, error instanceof Error ? error : undefined);
      setFeedState(prev => ({
        ...prev,
        isLoading: false,
        isLoadingMore: false,
  ...
  const interactWithPost = useCallback(async (interaction: PostInteraction) => {
    ...
    try {
      ...
      logger.error('Interaction error', { component: 'use-feed' }, error instanceof Error ? error : undefined);
