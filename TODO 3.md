...
### 5. ✅ Rate Limiting on Auth Routes - FIXED
...
- `resend-magic-link`: 3 req/min + progressive delays (30s→60s→120s→300s)
...
### ✅ Feed Fixes (This Session)
- ✅ **Privacy enforcement in feed** - Feed now only shows posts from:
  - Public/open spaces
  - Spaces the user is a member of
  - Posts without a spaceId (campus-wide)
- ✅ **Moderation filtering** - Feed skips hidden/deleted content
- ✅ **Error boundaries** - Added to feed, spaces, tools, profile routes
- ✅ **Search verified** - Search route uses real Firestore queries (not mock!)
